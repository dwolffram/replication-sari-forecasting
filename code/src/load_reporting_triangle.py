...
def rename_columns(ts):
    names = [('' if 'value' in x else 'cumsum_') + '_'.join(y) for x,y in zip(ts.components, ts.static_covariates_values())]
...
def load_reporting_triangle(target='sari', add_cumsum=True, shift=True):
    ...
    df = pd.wide_to_long(df, stubnames = value_cols, sep = '_',  suffix = r'.*',
                    i = ['location', 'age_group', 'year', 'week', 'date'], j = 'horizon').reset_index()
    ...
    for c in value_cols:
        ...
        ts = TimeSeries.from_group_dataframe(df_temp, group_cols=['location', 'age_group', 'horizon'], 
                                                 time_col='date', value_cols=[c], 
