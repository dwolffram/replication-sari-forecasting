# Helper functions to apply the KIT-hhh4 model
...
aggregate_forecasts_to_quantiles <- function(mu_matrix, var_matrix, quantile_levels_nowcast, quantile_levels, type = "linear pool"){
  if(type == "linear pool"){
    return(linear_pool(mu_matrix = mu_matrix, var_matrix = var_matrix,
                       quantile_levels_nowcast = quantile_levels_nowcast, 
                       quantile_levels = quantile_levels))
  }
  
  if(type == "quantile average"){
    return(quantile_average(mu_matrix = mu_matrix, var_matrix = var_matrix,
                            quantile_levels_nowcast = quantile_levels_nowcast, 
                            quantile_levels = quantile_levels))
  }
...
linear_pool <- function(mu_matrix, var_matrix, quantile_levels_nowcast, quantile_levels){
  ...
  quantile_matrix <- matrix(nrow = length(horizons), ncol = length(quantile_levels))
...
#'  means and variances for different nowcast paths
#'  Arguments as in aggregate_forecasts_to_quantiles
quantile_average <- function(mu_matrix, var_matrix, quantile_levels_nowcast, quantile_levels){
  horizons <- 1:nrow(mu_matrix)
  quantile_matrix <- matrix(nrow = length(horizons), ncol = length(quantile_levels))
  
  # get CDFs
  for(h in horizons){
    quantile_matrix_h <- matrix(NA, ncol = length(quantile_levels_nowcast), nrow = length(quantile_levels))
...
format_forecasts <- function(quantile_matrix, means, forecast_date, target_end_dates, 
                             location, age_group, horizons, quantile_levels){
  ...
  for(i in seq_along(horizons)){
    ...
    quantiles_formatted_h <- data.frame(location = rep(location, n_quantile_levels),
    ...
    # append:
    all_formatted <- rbind(all_formatted, quantiles_formatted_h, mean_formatted_h)
  }
